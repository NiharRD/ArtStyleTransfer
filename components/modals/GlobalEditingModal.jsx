...
const GlobalEditingModal = ({ visible, onClose, onHeightChange }) => {
  ...
  // Confirmation row visibility ("Keep these changes?")
  ...
  const getModalHeight = () => {
    let baseHeight = 330; // Base height with controls row + text input + actions row
  ...
  const handleTalkToKimiPress = () => {
    if (inputState === "textInput") {
      ...
      // Hide upper section when switching to voice prompt
      setUpperSectionVisible(false);
